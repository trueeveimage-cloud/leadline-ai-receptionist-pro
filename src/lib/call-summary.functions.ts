...
const SummarySchema = z.object({
  ...
  job_type: z.string().describe("Concrete service / product / job they need (e.g. 'leaking radiator', 'wisdom-tooth check')."),
  urgency: z.enum(["emergency", "high", "medium", "low"]).describe("Urgency of the request."),
  preferred_time: z.string().describe("When they want to be helped, e.g. 'Tue 10:30' or 'ASAP'. 'Flexible' if unclear."),
  location: z.string().describe("Address / area mentioned, else 'Not given'."),
  qualified: z.boolean().describe("True if this looks like a serious, bookable lead (not spam/wrong number)."),
  next_step: z.string().describe("One short imperative sentence for the owner, e.g. 'Call back today to confirm Tue 10:30 booking.'"),
  summary: z.string().describe("2–3 sentence natural-language summary the owner can read in 5 seconds."),
});
...
export const summarizeCall = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<CallSummary> => {
    ...
    const { experimental_output } = await generateText({
      ...
      prompt:
        `${businessLine}Conversation language: ${langName}.\n\nTranscript:\n${transcriptText}\n\nReturn the structured call summary.`,
    });
